<?php
    ...
    $query = "SELECT outlet.*, auth.nama_user, auth.id_user FROM outlet LEFT JOIN auth ON auth.outlet_id = outlet.id_outlet WHERE id_outlet  = " . $_GET['id'];
    $data = mysqli_query($conn, $query);
    $edit = mysqli_fetch_assoc($data);


    $query2 = "SELECT auth.*, outlet.nama_outlet FROM outlet RIGHT JOIN auth ON auth.outlet_id = outlet.id_outlet WHERE auth.role = 'owner' ORDER BY auth.outlet_id ASC";
    ...
    if (isset($_POST['btn-simpan'])) {
        ...
        if ($_POST['owner_new_id']) {
            $query2 = "UPDATE auth SET outlet_id = '" . $_GET['id'] . "' WHERE id_user = " . $_POST['owner_new_id'];
            $query3 = "UPDATE auth SET outlet_id = NULL WHERE id_user = " . $edit['id_user'];
            $update3 = mysqli_query($conn, $query3);
        } else {
            $query2 = "UPDATE auth SET outlet_id = '" . $_GET['id'] . "' WHERE id_user = " . $_POST['id_owner'];
        }
                    ...
                    <form action="" method="POST">
                        <div class="card-body">
                            <div class="form-group">
                                <label for="largeInput">Nama Cabang</label>
